...
!
   ...
   use ScfDataModule, only : ErBottom, ErTop, EiBottom, EiTop, Temperature, NumEs
...
!
   use BZoneModule, only : getNumKs, getAllKPoints
!
   use ProcMappingModule, only : getNumKsOnMyProc, getKPointIndex
...
!
!  *******************************************************************
   integer (kind=IntKind) :: LocalNumAtoms, NumAtoms, NumEsOnMyProc
...
!
   ...
   real (kind=RealKind), allocatable :: evec(:,:), kpts(:,:)
   real (kind=RealKind), pointer :: bravais(:,:)
   real (kind=RealKind) :: Efermi, kfac
!
...
!
   integer (kind=IntKind) :: i, id, ig, is, ie_loc, ie_glb, ik, k
   integer (kind=IntKind) :: eGID, aGID, NumPEsInGroup, comm, NumKs, NumKsOnMyProc
!
...
!
   ...
   allocate(kpts(3,NumKsOnMyProc))
   kvec => getAllKPoints(kfac)
   do ik = 1, NumKsOnMyProc
      k = getKPointIndex(ik)
      kpts(1:3,ik) = kvec(1:3,k)*kfac
   enddo
!  ===================================================================
!
!  The following part needs to be worked on...
!  +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
   do is = 1, n_spin_pola/n_spin_cant
!     ----------------------------------------------------------------
      call calBandStructure(is, ErBottom, ErTop, NumEs, kpts, NumKsOnMyProc)
!     ----------------------------------------------------------------
   enddo
!  +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!  End of the change
      ...
      do id = 1, LocalNumAtoms
         ...
         do is = 1,n_spin_pola
            ...
            do l = 0, lmax_pot(id)
               do m = 0, l
                  jl = jl + 1
                  if (.not.isPotComponentZero(id,is,jl)) then
